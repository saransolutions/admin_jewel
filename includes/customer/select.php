...
function get_main_table()
{
    ...
    foreach ($rows as $result) {
        ...
        $data = $data . '<tr>
            ...
            <td><a href="customers.php?cid=' . $id . '">'. ucfirst($name) . '</a></td>
            <td>' . $address . '</td>
            <td>' . $ort . '</td>
            <td>' . $mobile . '</td>
            <td>' . $dob . '</td>
...
function get_member_trans($id)
{
    $sql = "select id from ".DB_NAME.".members where customer_id = ".$id;
    $member_id = getSingleValue($sql);
    
    $sql = "select s.* from ".DB_NAME.".members m, ".DB_NAME.".schemes s where m.parent_id = s.id and m.id = ".$member_id;
    $scheme = getFetchArray($sql)[0];
    $part1 = '<table class="table table-bordered" cellspacing="0" width="100%" id="dataTable" cellspacing="0">
    <thead>
        <tr>
            <th>#</th>
    ...
    <tbody>
        ';
    $data = '';
    $sql = "select * from " . DB_NAME . ".member_trans where parent_id = ".$member_id." ORDER BY id DESC";
    $rows = getFetchArray($sql);
    $count = 1;
...
function get_single($id, $page){
    $sql = "select * from ".$page["table"]." where id = ".$id;
    $part1 = '<table class="table">
    <thead colspan="2" style="font"><strong style>'.$page["title"].' Details</strong></thead>
        <tbody>
        ';
    $data = '';
    $rows = getFetchArray($sql);
    foreach ($rows as $result) {
        ...
        $data .= '<tr><td>Prefix</td><td>'.$prefix.'</td></tr>';
        ...
        $data .= '<tr><td>Dob</td><td>'.$dob.'</td></tr>';
        $data .= '<tr><td>Joined Date</td><td>'.$joined_date.'</td></tr>';
